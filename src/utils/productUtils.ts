...
import { filterProducts } from './filterProductUtils';
...
export const getUniqueVolumeVariants = (
  ...
  return volumes.map((volume) => {
    ...
    const candidates = matched.filter((p) => p.attributes?.volume === volume);
...
export const getUniqueColorVariants = (
  ...
  return colors.map((color) => {
    ...
    const candidates = matched.filter((p) => p.attributes?.color === color);
